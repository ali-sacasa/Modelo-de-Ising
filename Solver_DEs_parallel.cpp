...
SystemConfig::SystemConfig() :
    ...
    output_dir("results/"), num_threads(omp_get_max_threads())
{
    generate_time_list();
}


void SystemConfig::set_num_threads(int n) {
    num_threads = (n > 0) ? n : omp_get_max_threads();
...
MatrixXcd CoherentStates::build_product_coherent_vectors(int N, const std::vector<Complex>& z_list) {
    ...
    #pragma omp parallel for schedule(dynamic) // dynamic hace que los hilos trabajen conforme se van desocupando. Es decir, no todos los hilos tienen la misma cantidad de its.
    for (int j = 0; j < S; ++j) {
...
RicciFlow::RicciResults RicciFlow::optimize_projection(
    ...
    // Almacenamiento temporal para resultados paralelos
    std::vector<MatrixXcd> P_list(n_beta);
    std::vector<std::vector<double>> eigenvalues_list(n_beta);
    ...
    // *** Búsqueda del mejor resultado (secuencial después del paralelo) ***
    for (int i = 0; i < n_beta; ++i) {
        if (results.fidelities[i] > best_fidelity) {
            best_fidelity = results.fidelities[i];
            best_idx = i;
        }
    }
    
    results.best_P = P_list[best_idx];
    results.best_beta = beta_list[best_idx];
    results.best_fidelity = best_fidelity;
...
std::pair<MatrixXcd, MatrixXcd> ChebyshevPropagator::project_and_evolve(
    ...
    std::cout << "Evolucionando con Chebyshev (paralelo)...\n";
...
MatrixXcd EnergyAnalyzer::compute_local_energy_density(
    const MatrixXcd& states, 
    ...
    #pragma omp parallel for collapse(2) // Aquí se paralelizan los dos for anidades, haciendo que no se paralelicen solo los tiempos, si no también el sitio de cada espín. Es hace que cada elemento de la matriz se calcule de forma independiente.
    for (int i = 0; i < nt; ++i) {
        for (int j = 0; j < N; ++j) {
            VectorXcd state = states.col(i);
...
void CSVExporter::save_matrix_csv(const Eigen::MatrixXd& data,
                                 const std::string& filename,
                                 const std::string& header) {
    std::ofstream file(filename);
    if (!file.is_open()) {
...
void CSVExporter::save_complex_matrix_csv(const Eigen::MatrixXcd& data,
                                         const std::string& filename,
                                         const std::string& header) {
    std::ofstream file(filename);
    if (!file.is_open()) {
